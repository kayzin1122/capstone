function CustomersSay() {
    return (
        <section>
            <h2>What our customers say</h2>
            <p>We have a lot of satisfied customers, here are some of their testimonials:</p>
            <ul>
                <li>
                    <p>"The 'Chef's Mystery Creation' was a delightful surprise! Incredibly flavorful and unique. Highly recommend trying it!"</p>
                    <p>- Sarah M.</p>
                </li>
                <li>
                    <p>"Best steak I've had in a while! Perfectly cooked and so tender. A must-try for steak lovers."</p>
                    <p>- Jane D.</p>
                </li>
                <li>
                    <p>"Classic Bolognese done right! Rich, comforting, and delicious. A family favorite!"</p>   
                    <p>- Mark K.</p>
                </li>
            </ul>
        </section>
    );
}

export default CustomersSay;